...
loader.load(
    ...
    function (gltf) {
        ...
        // Debug: show bounding box helper for the loaded model (before/after instancing)
        try {
            ...
            const boxHelper = new THREE.Box3Helper(debugBox, 0xff0000);
            scene.add(boxHelper);

            // Auto-frame camera if model is far or small
            const maxDim = Math.max(size.x, size.y, size.z);
            if (maxDim > 0) {
                // position camera to look at center from a distance based on model size
                const distance = Math.max(200, maxDim * 2.5);
                camera.position.set(center.x, center.y + Math.max(50, maxDim * 0.5), center.z + distance);
                if (controls && typeof controls.target !== 'undefined') controls.target.copy(center);
                console.log('Auto-framed camera to', camera.position, 'target', center);
            }
        } catch (err) {
            console.warn('Bounding box debug failed', err);
        }
    },
    // progress
    function (xhr) {
        try {
            if (xhr.lengthComputable) {
                ...
                console.log(`GLTF load progress: ${pct}%`);
            } else {
...
function onPointer(event) {
    ...
    // Read current matrix, modify scale, and set matrix back
    ...
    if (!isSelected) {
        ...
        // revert to base scale if known
        const base = instancedBaseScale.get(inst.uuid) || new THREE.Vector3(1,1,1);
        dummy.position.z += -80;
...
// Click to select (use 'click' so dragging with the mouse doesn't trigger selection)
