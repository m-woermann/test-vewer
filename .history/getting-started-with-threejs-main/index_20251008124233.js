...
loader.load(
    ...
    function (gltf) {
        ...
        // Create InstancedMeshes: 6 speakers side-by-side
        const instances = 6;
        const vinstances = 2;
        const spacing = 270;
        const vspacing = 500 // distance between speaker centers on X axis
        ...
        model.traverse((child) => {
            if (child.isMesh && child.geometry) {
                ...
                const inst = new THREE.InstancedMesh(geom, material, instances , vinstances);
                inst.castShadow = true;
                inst.receiveShadow = true;

                const dummy = new THREE.Object3D();

                for (let j = 0; j < vinstance; j++) {

                for (let i = 0; i < instances; i++) {
                    // Position each instance along X
                    dummy.position.set((i - (instances - 1) / 2) * spacing, (j - (vinstances - 1) / 2) * vspacing, child.position.z);
...
// Only allow horizontal pan via drag (disable OrbitControls rotate)
...
const panSpeed = 0.5; // world units per pixel (used to compute initial velocity)
let panVelocity = 0; // current pan velocity (world units per second)
const panDamping = 5.0; // damping coefficient (higher = faster stop)
...
function onPointerMovePan(e) {
    ...
    // set pan velocity based on latest pointer delta
    // convert pixel delta into a velocity (world units per second)
    panVelocity = -dx * panSpeed * 60; // approximate per-second velocity
}
...
const animate = () => {
    ...
    const delta = (typeof THREE !== 'undefined' && THREE.Clock) ? (clock ? clock.getDelta() : 0.016) : 0.016;
    // apply pan velocity with damping
    if (Math.abs(panVelocity) > 1e-4) {
        const forward = new THREE.Vector3();
        camera.getWorldDirection(forward);
        const right = new THREE.Vector3().crossVectors(forward, camera.up).normalize();
        const move = panVelocity * delta;
        camera.position.addScaledVector(right, move);
        controls.target.addScaledVector(right, move);
        // damp the velocity
        panVelocity -= panVelocity * panDamping * delta;
    }
    controls.update();
