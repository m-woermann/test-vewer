...
const debugMarkerGeometry = new THREE.SphereGeometry(60, 12, 12); // larger so it's easier to see
const debugMarkerMaterial = new THREE.MeshBasicMaterial({ color: 0xff0000 });
debugMarkerMaterial.depthTest = false; // render on top
debugMarkerMaterial.depthWrite = false;
...
function selectInstance(inst, instanceId) {
    ...
    // nudge upward so it sits above the model and remains visible
    debugMarker.position.y += 60;
    debugMarker.visible = true;
    const camDist = camera.position.distanceTo(worldPos).toFixed(2);
    console.log('[selectInstance] worldPos after set:', worldPos.toArray(), 'camDist:', camDist);
}
...
function deselectInstance(inst, instanceId) {
    ...
    debugMarker.position.y += 60;
    const camDist = camera.position.distanceTo(worldPos).toFixed(2);
    console.log('[deselectInstance] worldPos after restore:', worldPos.toArray(), 'camDist:', camDist);
