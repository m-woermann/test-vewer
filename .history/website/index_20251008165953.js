...
spotLight.position.set(0, 800,3000);
...
function onPointerMovePan(e) {
    ...
    // also move the spotlight and its target so the light follows the camera during immediate pans
    spotLight.position.addScaledVector(right, amount);
    targets1.position.addScaledVector(right, amount);
}
...
const animate = () => {
    ...
    if (Math.abs(panVelocity) > 1e-3) {
        ...
        spotLight.position.addScaledVector(right, move);
        targets1.position.addScaledVector(right, move);
