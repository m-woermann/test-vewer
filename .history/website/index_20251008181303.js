...
function selectInstance(inst, instanceId) {
    ...
    // Store original matrix if not already saved
    ...
    if (originals && !originals[instanceId]) {
        ...
        originals[instanceId] = matrix.clone();
    }
    ...
    inst.setMatrixAt(instanceId, matrix);
...
function onPointer(event) {
    ...
    for (const inst of instancedMeshes) {
        ...
        if (hits && hits.length > 0) {
            if (hits[0].distance < closestDist) {
                closest = { hit: hits[0], inst };
    ...
    if (!isSelected) {
        // Deselect any previously selected instances
        for (const otherInst of instancedMeshes) {
            const otherSelectedSet = selectedInstances.get(otherInst.uuid);
            if (otherSelectedSet && otherSelectedSet.size > 0) {
        ...
        // Deselect if clicking already selected instance
