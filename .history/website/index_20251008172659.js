...
function onPointer(event) {
    ...
    // If selecting a new instance (not toggling the same instance), revert all other selected instances
    // across all instanced meshes back to their original matrices.
    let selectingDifferent = true;
    // determine if the clicked instance is the only selected one on its mesh and whether we are toggling it
    if (isSelected) selectingDifferent = false;

    if (selectingDifferent) {
